//UserProfile User Controller Logic:-

import UserRole from "../../Models/UserModel.js";


//create profile controller:-
export const CreateProfileRole = async(req, res) => {
    try {
        ...
        const existProfile = await UserRole.findOne({userId});
        if(existProfile){
            return res.status(400).json({
                success: false,
        ...
        const newUserprofile = await UserRole.create({userId, username, bioDesc, phone, email, profileImage: cleanImagePath});
...
export const GetProfileRole = async(req, res) => {
    try {
        const userId = req.user.id;
        
        //find user profile by id:-
        const fetchprofile = await UserRole.findOne({userId: userId});
        if(!fetchprofile){
...
export const UpdateProfileRole = async(req, res) => {
    try {
        ...
        //Updating:-
        const UpdateProfile = await UserRole.findOneAndUpdate({userId}, updateData, {returnDocument: "after"});
        if(!UpdateProfile){
            return res.status(404).json({
                success: false,
